...
def generate_excel_report_by_months(user_id, username, today_only=False):
    ...
    # Оставляем только сегодня, если нужно
    if today_only:
        today = datetime.now().date()
        m = today.month
        today_rec = data_by_month.get(m, {}).get(today, {
            "start": None, "lunch_out": None,
            "lunch_in": None, "end": None
        })
        data_by_month = {m: {today: today_rec}}
    ...
    for month, records in data_by_month.items():
        ...
        for date, t in sorted(records.items()):
            st, lo, li, en = t["start"], t["lunch_out"], t["lunch_in"], t["end"]
            minutes = late_flag = 0
            if st and en:
                total_days += 1
